...
# personality inheritance: parent symlinks are replicated into the sub-profile
SHARED="$ROOT/shared-commands"; mkdir -p "$SHARED"; print -r -- "x" > "$SHARED/pickup.md"
ln -s "$SHARED" "$PARENT/commands"
print -r -- "profile note" > "$PARENT/CLAUDE.md"
( cd "$ROOT/work" && pp add-login d ) >/dev/null 2>&1
SUBD="$ROOT/.claude-work-d"
[ -L "$SUBD/commands" ] && [ "$(readlink "$SUBD/commands")" = "$SHARED" ] && ok "parent symlink replicated into sub-profile" || bad "symlink not replicated"
[ -f "$SUBD/commands/pickup.md" ] && ok "shared command visible through replicated link" || bad "shared command not visible"
[ -f "$SUBD/CLAUDE.md" ] && ok "per-profile CLAUDE.md inherited" || bad "CLAUDE.md not inherited"
[ ! -f "$SUBD/.credentials.json" ] && ok "still no credentials copied" || bad "CREDENTIALS COPIED via personality inherit"
...
if [[ "$OSTYPE" == darwin* ]]; then
  ...
  mkdir -p "$ROOT/.claude-work-g"
  out="$( cd "$ROOT/work" && pp capture g 2>&1 )"
